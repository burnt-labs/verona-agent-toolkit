...
# - Audit logging
#
# Usage (from verona-treasury/scripts/*.sh):
...
log_audit() {
    ...
    if [[ "$VERONA_AUDIT_LOG" == "true" ]]; then
        timestamp=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
        
        # Ensure log directory exists
        mkdir -p "$(dirname "$VERONA_AUDIT_LOG_FILE")" 2>/dev/null || true
        
        # Append to audit log
        echo "[$timestamp] $operation: $details" >> "$VERONA_AUDIT_LOG_FILE" 2>/dev/null || true
    fi
}
...
confirm_operation() {
    ...
    if [[ "$VERONA_SKIP_CONFIRM" == "true" ]] || [[ ! -t 0 ]]; then
...
get_confirmation_env() {
    if [[ "$VERONA_SKIP_CONFIRM" == "true" ]]; then
        echo "skip"
    elif [[ -t 0 ]]; then
        echo "interactive"
    else
