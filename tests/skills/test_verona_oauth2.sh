#!/bin/bash
#
# tests/skills/test_verona_oauth2.sh - OAuth2 Skill Tests
#
# Tests for verona-oauth2 skill including login, status, logout, and refresh operations.
#
# Usage:
#   MOCK_ENABLED=true ./test_verona_oauth2.sh
#
# Environment:
...
test_oauth2_status_authenticated() {
    ...
    assert_json_has_key "$result" ".verona_address" || return 1
...
test_oauth2_login_success() {
    ...
    local address
    address=$(json_get "$result" ".verona_address")
